...
class MonteCarlo(object):
    ...
    def choose_move(self):
        ...
        moves_states = []
        for move in moves: 
            piece_movement = self.game.make_move(move, store=False, update_hash=True)
            moves_states.append((move, self.game.hash))
            self.game.undo_move(piece_movement, remove=False)
        ...
        return (self.game.board[move[0]], self.game.board[move[1]])
    ...
    def run_simulation(self):
        ...
        move_count = 1
        expand = True
        while not game_copy.is_finished() and move_count < self.max_moves + 1:
            if player == -1: 
                game_copy.next_turn()
                player = game_copy.get_turn(auto_play_bots=False)
                continue
            ...
            for move in moves: 
                piece_movement = game_copy.make_move(move)
                moves_states.append((move, game_copy.hash))
                game_copy.undo_move(piece_movement)
